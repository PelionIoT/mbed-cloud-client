...
static ds_status_e start_metrics_collection_config_message_handle(CborValue *metric_configuration_data)
{
    ...
    // move container iterator to next element
    CborError cbor_err = cbor_get_next_container_element(metric_configuration_data);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((cbor_err != CborNoError), DS_STATUS_INVALID_CONFIG, "Failed to move to next element");
    ...
    cbor_err = cbor_value_enter_container(metric_configuration_data, &single_config);
...
{
    SA_PV_LOG_INFO("Setting cb=%p, context=%p", cb, user_context);
