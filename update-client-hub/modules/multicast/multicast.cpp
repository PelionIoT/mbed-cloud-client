...
#define RECEIVE_BUFFER_SIZE                 21 + OTA_FRAGMENT_SIZE // OTA_FRAGMENT_CMD_LENGTH 21 bytes + fragment size 512 bytes
...
static void             arm_uc_multicast_send_update_fw_cmd_received_info(uint16_t delay);
static int8_t           arm_uc_multicast_socket_send(ota_ip_address_t* destination, uint16_t count, uint8_t* payload);
static uint16_t         arm_uc_multicast_coap_send_notif(ota_resource_types_e type, uint8_t *payload_ptr, uint16_t payload_len);
...
int arm_uc_multicast_init(M2MBaseList& list, ConnectorClient& client, const int8_t tasklet_id)
{
    ...
    if (ota_lib_configure(&arm_uc_multicast_ota_config, &arm_uc_ota_function_pointers, 1) != OTA_OK) {
        return -1;
    }
    ...
    memcpy(arm_uc_multicast_ota_config.unicast_socket_addr.address_tbl, stack_info.parent, 16);
    tr_info("arm_uc_multicast_init - parent address: %s", trace_ipv6(arm_uc_multicast_ota_config.unicast_socket_addr.address_tbl));
...
{
    tr_debug("arm_uc_multicast_socket_callback - port %ld", (intptr_t)port);
...
void arm_uc_multicast_tasklet(struct arm_event_s *event)
{
    if (ARM_UC_OTA_MULTICAST_TIMER_EVENT == event->event_type) {
        ...
        memset(stored_ota_parameters.ota_session_id, 0, 16);
...
static void arm_uc_multicast_request_timer(uint8_t timer_id, uint32_t timeout)
{
    tr_info("arm_uc_multicast_request_timer - id %" PRIu8 ", timeout %" PRIu32 " ", timer_id, timeout);
...
static uint32_t arm_uc_multicast_write_fw_bytes(uint8_t *ota_session_id, uint32_t offset, uint32_t count, uint8_t* from)
{
    ...
    buffer.size_max = OTA_FRAGMENT_SIZE;
    buffer.size = count;
    buffer.ptr = from;
...
static bool arm_uc_multicast_create_static_resources()
{
    ...
    if (arm_uc_multicast_object_inst) {
        ...
        if (arm_uc_multicast_ota_config.device_type == OTA_DEVICE_TYPE_BORDER_ROUTER) {
            multicast_connected_nodes_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_ota_connected_nodes_res, M2MResourceInstance::INTEGER);
            if (!multicast_connected_nodes_res) {
            ...
            multicast_ready_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_ota_ready_for_multicast_res, M2MResourceInstance::INTEGER);
            if (!multicast_ready_res) {
            ...
            multicast_command_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_ota_command_res, M2MResourceInstance::OPAQUE);
            if (!multicast_command_res) {
                tr_error("arm_uc_multicast_create_static_resources - failed to create multicast_command_res!");
                return false;
            }

            multicast_estimated_total_time_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_estimated_total_time_res, M2MResourceInstance::OPAQUE);
            if (!multicast_estimated_total_time_res) {
            ...
            multicast_estimated_resend_time_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_estimated_resend_time_res, M2MResourceInstance::OPAQUE);
            if (!multicast_estimated_resend_time_res) {
        ...
        multicast_netid_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_dodag_id_res, M2MResourceInstance::STRING);
        ...
        multicast_status_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_ota_status_res, M2MResourceInstance::OPAQUE);
        if (!multicast_status_res) {
            tr_error("arm_uc_multicast_create_static_resources - failed to create multicast_status_res!");
            return false;
        }

        multicast_session_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_ota_session_res, M2MResourceInstance::OPAQUE);
        if (!multicast_session_res) {
            tr_error("arm_uc_multicast_create_static_resources - failed to create multicast_session_res!");
            return false;
        }

        multicast_error_res = arm_uc_multicast_object_inst->create_static_resource(&arm_uc_multicast_error_res, M2MResourceInstance::OPAQUE);
...
static ota_error_code_e arm_uc_multicast_start_received(ota_parameters_t* ota_parameters)
{
    ...
    if (!arm_uc_multicast_fwmanager_prepared) {
        ...
        if (ret.code == ERR_NONE) {
            ...
            tr_warn("ARM_UC_FirmwareManager.Prepare failed with %" PRId32 " ", ret.code);
        }
    }
    else {
        return_value = OTA_OK;
