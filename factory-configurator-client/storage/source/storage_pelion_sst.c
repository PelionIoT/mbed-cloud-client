    ...
    size_t * kcm_item_data_size_out)
{
    ...
    if (kcm_item_type == KCM_CERTIFICATE_ITEM ||
        ...
        store_ext_secure_file_ctx_s ext_ctx;
        kcm_status = storage_file_open_external_certificate_store(&ext_ctx, &ctx);
        SA_PV_ERR_RECOVERABLE_GOTO_IF((kcm_status != KCM_STATUS_SUCCESS), (kcm_status = kcm_status), Exit, "Failed opening external secure file (%d)", kcm_status);

        kcm_data_size = ext_ctx.ext_secure_file_data_size;
...
#ifdef MBED_CONF_MBED_CLOUD_CLIENT_EXTERNAL_CERTIFICATE_STORE_SUPPORT
    // For certificates and keys, kcm_item_data_out contains external secure storage file location
    if (kcm_item_type == KCM_CERTIFICATE_ITEM ||
        kcm_item_type == KCM_PRIVATE_KEY_ITEM ||
        kcm_item_type == KCM_PUBLIC_KEY_ITEM) {
        ...
        if (kcm_item_data_max_size > 0) {
            if (kcm_item_data_max_size < ext_ctx.ext_secure_file_data_size) {
                storage_file_close_external_certificate_store(&ext_ctx);
                kcm_status = KCM_STATUS_INSUFFICIENT_BUFFER;
                goto Exit;
            }
            memcpy(kcm_item_data_out, ext_ctx.ext_secure_file_data, ext_ctx.ext_secure_file_data_size);
        }
        *kcm_item_data_act_size_out = ext_ctx.ext_secure_file_data_size;
...
kcm_status_e storage_cert_chain_get_next_size(kcm_cert_chain_handle *kcm_chain_handle, storage_item_prefix_type_e item_prefix_type, size_t *kcm_out_cert_data_size)
{
    ...
    SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed opening external secure file (%d)", kcm_status);

    *kcm_out_cert_data_size = chain_context->current_ext_ctx.ext_secure_file_data_size;
#endif // MBED_CONF_MBED_CLOUD_CLIENT_EXTERNAL_CERTIFICATE_STORE_SUPPORT
                                              ...
                                              size_t *kcm_actual_cert_data_size)
{
...
#ifdef MBED_CONF_MBED_CLOUD_CLIENT_EXTERNAL_CERTIFICATE_STORE_SUPPORT
    store_ext_secure_file_ctx_s *ext_ctx = &chain_context->current_ext_ctx;

    if (ext_ctx->ext_secure_file_data_size == 0) {
        kcm_status = KCM_STATUS_INSUFFICIENT_BUFFER;
        return kcm_status;
    }
    ...
    if (kcm_max_cert_data_size > 0) {
        if (kcm_max_cert_data_size < ext_ctx->ext_secure_file_data_size) {
            kcm_status = KCM_STATUS_INSUFFICIENT_BUFFER;
            return kcm_status;
        }

        memcpy(kcm_cert_data, ext_ctx->ext_secure_file_data, ext_ctx->ext_secure_file_data_size);

    }
    *kcm_actual_cert_data_size = ext_ctx->ext_secure_file_data_size;
...
kcm_status_e storage_cert_chain_close(kcm_cert_chain_handle kcm_chain_handle, storage_item_prefix_type_e item_prefix_type)
{
    ...
    if (chain_context->current_ext_ctx.ext_secure_file_data != NULL) {
...
*
*   @param ext_ctx External secure file context to populate
*   @param ctx ESFS file context that must be already opened
*
*   @returns
*       KCM_STATUS_SUCCESS in case of success otherwise one of kcm_status_e errors
*/
kcm_status_e storage_file_open_external_certificate_store(store_ext_secure_file_ctx_s *ext_ctx,
                                         store_esfs_file_ctx_s *ctx)
{
    kcm_status_e kcm_status = KCM_STATUS_SUCCESS;
    ...
    memset(ext_ctx, 0, sizeof(store_ext_secure_file_ctx_s));
    uint8_t ext_secure_file_name[STORAGE_FILENAME_MAX_SIZE];

    // Read esfs_file_h using storage_file_read_with_ctx
    kcm_status = storage_file_read_with_ctx(ctx, ext_secure_file_name, STORAGE_FILENAME_MAX_SIZE, &ext_ctx->ext_secure_file_name_size);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed reading esfs (kcm_status %d)", kcm_status);

    // Ensure null termination
    if (ext_ctx->ext_secure_file_name_size > 0) {
        ext_secure_file_name[ext_ctx->ext_secure_file_name_size] = '\0';
    }

    if (ext_secure_file_name[0] == '\0') {
        kcm_status = KCM_STATUS_ITEM_NOT_FOUND;
        return kcm_status;
    }

    // Read file size from external secure storage area
    kcm_status = storage_read_file_size_from_external_certificate_store(ext_secure_file_name,
                                            ext_ctx->ext_secure_file_name_size,
                                            &ext_ctx->ext_secure_file_data_size);
    ...
    ext_ctx->ext_secure_file_data = malloc(ext_ctx->ext_secure_file_data_size);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((ext_ctx->ext_secure_file_data == NULL), KCM_STATUS_OUT_OF_MEMORY, "Failed allocating memory for external secure file data");

    // Read file from external secure storage area
    kcm_status = storage_read_file_from_external_certificate_store(ext_secure_file_name,
                                            ext_ctx->ext_secure_file_name_size,
                                            ext_ctx->ext_secure_file_data,
                                            ext_ctx->ext_secure_file_data_size,
                                            &ext_ctx->ext_secure_file_data_size);
    SA_PV_ERR_RECOVERABLE_RETURN_IF((kcm_status != KCM_STATUS_SUCCESS), kcm_status, "Failed reading external secure file (kcm_status %d)", kcm_status);
...
*
*   @param ext_ctx External secure file context to close
*
*   @returns
*       KCM_STATUS_SUCCESS in case of success otherwise one of kcm_status_e errors
*/
kcm_status_e storage_file_close_external_certificate_store(store_ext_secure_file_ctx_s *ext_ctx)
{
    ...
    if (ext_ctx->ext_secure_file_data != NULL) {
        free(ext_ctx->ext_secure_file_data);
    }
