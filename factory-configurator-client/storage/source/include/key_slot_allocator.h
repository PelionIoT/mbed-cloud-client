...
/*===========================Generatl Purpose APIs===========================================*/
    ...
    */
    ...
    /** Loads an external key resides in secure element into a new table entry.
    *
    * If the target key already exist in table the function will verify that the key origin is secure element,
    * if not, KCM_STATUS_ERROR will be returned.
    *
    * @key_name[IN] The complete key name (hash + metadata)
    * @key_id[IN] The key identifier as originated by secure element
    * @is_factory[IN] set to "true" if this item is factory, "false" otherwise
    *
    * @returns ::KCM_STATUS_SUCCESS if the key exist in store and the `key_location_out` parameter was set `::kcm_status_e` errors in case of an error.
    * @        ::KCM_STATUS_ERROR if the given key already exist but its location is not KCM_LOCATION_SECURE_ELEMENT.
    */
