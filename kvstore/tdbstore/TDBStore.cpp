...
static uint32_t calc_crc(uint32_t init_crc, uint32_t size, const void *buffer)
{
    ...
    // These combined do nothing?
    crc = crc & 0xFFFFFFFF;
    crc = (crc ^ 0x0) & 0xFFFFFFFF;
...
int TDBStore::set_add_data(set_handle_t handle, const void *value_data, size_t data_size)
{
    ...
    pal_osMutexWait(_inc_set_mutex, PAL_RTOS_WAIT_FOREVER);
    ...
    pal_osMutexRelease(_inc_set_mutex);
    return ret;
}
...
int TDBStore::garbage_collection()
{
    ...
    if (!ret) {
        ...
        while (reserved_size) {
            chunk_size = std::min(_prog_size, reserved_size);
...
int TDBStore::init()
{
    ...
    pal_osMutexCreate(&_mutex);
    pal_osMutexCreate(&_inc_set_mutex);
...
int TDBStore::do_reserved_data_get(void *reserved_data, size_t reserved_data_buf_size, size_t *actual_data_size)
{
    ...
    while (actual_size) {
        uint32_t chunk = std::min(_prog_size, (uint32_t) actual_size);
...
int TDBStore::offset_in_erase_unit(uint8_t area, uint32_t offset,
...
int TDBStore::is_erase_unit_erased(uint8_t area, uint32_t offset, bool &erased)
{
    ...
    int ret = offset_in_erase_unit(area, offset, offset_from_start, dist);
    if (ret) {
        return MBED_ERROR_READ_FAILED;
    }
    ...
    while (dist) {
        ...
        ret = read_area(area, offset, chunk, buf);
        if (ret) {
            return MBED_ERROR_READ_FAILED;
        }
