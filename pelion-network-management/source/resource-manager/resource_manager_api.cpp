...
void handle_stats_coap_request(const M2MBase &base,
                                     ...
                                     void *client_args)
{
    ...
    if (status == NM_STATUS_SUCCESS) {
        tr_debug("Resource data length = %u", len);
#if (ENABLE_DEBUG_PRINT_STREAM)
        if(obj == routing_table) {
            print_stream((uint8_t *)buffer, len > 32 ? 32 : len);
        } else {
            print_stream((uint8_t *)buffer, len);
        }
#endif
        if (obj->send_async_response_with_code(buffer, len, token, token_len, COAP_RESPONSE_CHANGED) != true) {
            tr_err("FAILED: To send resource data to Cloud Client");
            nm_dyn_mem_free(radio_stats_buf);
            return;
        }
        tr_info("Resource value Sending to Cloud Client");
        return;
    }
    tr_warn("FAILED: To fetch resource information");
}
...
nm_status_t nm_res_manager_create(void *obj_list)
{
    ...
    app_stats->set_async_coap_request_cb(handle_stats_coap_request, app_stats);
    ...
    nm_stats->set_async_coap_request_cb(handle_stats_coap_request, nm_stats);
    ...
    ws_stats->set_async_coap_request_cb(handle_stats_coap_request, ws_stats);
    ...
    ch_noise->set_async_coap_request_cb(handle_stats_coap_request, ch_noise);
    ...
    if (MBED_CONF_MBED_MESH_API_WISUN_DEVICE_TYPE == MESH_DEVICE_TYPE_WISUN_BORDER_ROUTER) {
        ...
        br_stats->set_async_coap_request_cb(handle_stats_coap_request, br_stats);
        ...
        routing_table->set_async_coap_request_cb(handle_stats_coap_request, routing_table);
    }
    if (MBED_CONF_MBED_MESH_API_WISUN_DEVICE_TYPE == MESH_DEVICE_TYPE_WISUN_ROUTER) {
        ...
        node_stats->set_async_coap_request_cb(handle_stats_coap_request, node_stats);
        ...
        radio_stats->set_async_coap_request_cb(handle_stats_coap_request, radio_stats);
    }
    return NM_STATUS_SUCCESS;
}
