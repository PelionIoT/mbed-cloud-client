...
#define TRACE_GROUP "iface_mgr"

#define WS_CONF_MAX_BUF WS_CONF_MAX_ENCCODER_BUF
#define BR_CONF_MAX_BUF BR_CONF_MAX_ENCCODER_BUF
#define WS_STAT_MAX_BUF WS_STAT_MAX_ENCCODER_BUF
#define NM_STAT_MAX_BUF NM_STAT_MAX_ENCCODER_BUF
#define BR_STAT_MAX_BUF BR_STAT_MAX_ENCCODER_BUF
#define NI_STAT_MAX_BUF NI_STAT_MAX_ENCCODER_BUF
#define RQ_STAT_MAX_BUF RQ_STAT_MAX_ENCCODER_BUF

#define ROUTE_INFO_MIN_BLOCK_SIZE 512

static SocketAddress sa;
static NetworkInterface *backhaul_interface = NULL;
static NetworkInterface *mesh_network = NULL;
static WisunInterface *ws_iface = NULL;
static WisunBorderRouter ws_br;
static bool interface_connected = false;
static bool border_router_started = false;

typedef struct br_routing_info_mem {
    void *buf;
    uint16_t size;
} br_routing_info_mem_t;
static br_routing_info_mem_t route_info_mem;
...
/*************************Backhaul Interface*****************************/
static void backhaul_network_status_callback(nsapi_event_t status, intptr_t param)
{
    nsapi_error_t ret = -1;
    if (status == NSAPI_EVENT_CONNECTION_STATUS_CHANGE) {
        switch (param) {
            case NSAPI_STATUS_GLOBAL_UP:
                ...
                ret = backhaul_interface->get_ip_address(&sa);
                if (ret != 0) {
                    tr_err("get_ip_address failed with %d", ret);
                    nm_post_event(NM_EVENT_BACKHAUL_CONNECTED, IFACE_STATUS_FAIL, NULL);
                } else {
                    printf("Backhaul Network initialized, connected with IP %s\n", sa.get_ip_address());
                    nm_post_event(NM_EVENT_BACKHAUL_CONNECTED, IFACE_STATUS_SUCCESS, backhaul_interface);
                }
                break;
...
void nm_iface_backhaul_up(void)
{
    ...
    if (backhaul_interface == NULL) {
        ...
        /* Sending BB Fail notification to network manager */
        nm_post_event(NM_EVENT_BACKHAUL_CONNECTED, IFACE_STATUS_FAIL, NULL);
        return;
    }

    // Delete the callback first in case the API is being called multiple times to prevent creation of multiple callbacks.
    backhaul_interface->remove_event_listener(mbed::callback(&backhaul_network_status_callback));
    backhaul_interface->add_event_listener(mbed::callback(&backhaul_network_status_callback));

    backhaul_interface->set_blocking(false);
...
/* To-Do :: check Trigger point for the following disconnect functions */
/*
 * "nm_iface_backhaul_down" function is called to down/disconnect the Backhaul interface
 */
void nm_iface_backhaul_down(void)
{
    backhaul_interface->disconnect();
    tr_info("Backhaul interface is disconnected");
}
/************************************************************************/
...
/***************************Mesh Interface*******************************/
static uint32_t get_ws_resource_version()
{
    return WS_RESOURCE_VERSION;
}

static nm_status_t update_ws_config_to_nanostack(nm_ws_config_t *existing_ws_config, nm_ws_config_t *updated_ws_config)
{
    ...
    updated_ws_config->resource_version = get_ws_resource_version();
    tr_info("WS_resource_version %lu", updated_ws_config->resource_version);
    /* Delay and Device_min_sense parameters are currently not used */
    updated_ws_config->delay = 0;
    updated_ws_config->device_min_sens = 0;
...
static nm_status_t nm_configure_wisun_iface(void)
{
    ...
    if (update_ws_config_to_nanostack(&default_ws_config, &modified_ws_config) == NM_STATUS_FAIL) {
...
static void mesh_network_status_callback(nsapi_event_t status, intptr_t param)
{
    if (status == NSAPI_EVENT_CONNECTION_STATUS_CHANGE) {
        switch (param) {
            case NSAPI_STATUS_GLOBAL_UP:
                tr_info("NSAPI_STATUS_GLOBAL_UP");
                nm_post_event(NM_EVENT_MESH_CONNECTED, IFACE_STATUS_SUCCESS, mesh_network);
            ...
            case NSAPI_STATUS_CONNECTING:
                tr_info("NSAPI_STATUS_CONNECTING");
                /*
                 * Following functions should be called from NSAPI_STATUS_GLOBAL_UP event.
                 * But NSAPI_STATUS_GLOBAL_UP event is not coming for Wi-SUN border router.
                 */
                if (MBED_CONF_MBED_MESH_API_WISUN_DEVICE_TYPE == MESH_DEVICE_TYPE_WISUN_BORDER_ROUTER) {
                    nm_post_event(NM_EVENT_MESH_CONNECTED, IFACE_STATUS_SUCCESS, mesh_network);
                    tr_info("Mesh interface connecting");
                    interface_connected = true;
                }
                break;
            case NSAPI_STATUS_ERROR_UNSUPPORTED:
...
nm_status_t nm_iface_mesh_init(void)
{
    ...
    tr_debug("Initialize Mesh Network");
    mesh_network = MeshInterface::get_default_instance();
    if (mesh_network == NULL) {
        tr_err("Failed to get default Mesh Network Interface");
        return NM_STATUS_FAIL;
    }
    return NM_STATUS_SUCCESS;
}
...
nm_status_t nm_iface_mesh_up(void)
{
    ...
    if (interface_connected == true) {
        tr_debug("Already connected to mesh network");
        return NM_STATUS_SUCCESS;
    }

    if (mesh_network == NULL) {
        tr_error("FAILED mesh_network is NULL");
        return NM_STATUS_FAIL;
    }
    ws_iface = reinterpret_cast<WisunInterface *>(mesh_network);

    if (nm_configure_wisun_iface() == NM_STATUS_FAIL) {
        tr_error("FAILED to configure WiSUN Interface");
        return NM_STATUS_FAIL;
    }

    ws_iface->enable_statistics();

    mesh_network->add_event_listener(mbed::callback(&mesh_network_status_callback));
    mesh_network->set_blocking(false);
...
/*
 *  "nm_iface_mesh_down" function is called to down/disconnect the Mesh interface
 */
void nm_iface_mesh_down(void)
{
    mesh_network->disconnect();
    tr_info("Mesh interface is disconnected");
}
...
nm_status_t nm_res_set_ws_config(uint8_t *data, size_t length)
{
    ...
    if (update_ws_config_to_nanostack(&kvstored_ws_config, &received_ws_config) == NM_STATUS_FAIL) {
        tr_warn("FAILED to set Wi-SUN config to Nanostack");
        return NM_STATUS_FAIL;
    }
...
/****************************BR Interface********************************/
static uint32_t get_br_resource_version()
{
    return WS_BR_RESOURCE_VERSION;
}

static nm_status_t update_br_config_to_nanostack(nm_br_config_t *existing_br_config, nm_br_config_t *updated_br_config)
{
    mesh_error_t status = MESH_ERROR_UNKNOWN;
    ...
    if (memcmp(existing_br_config->network_name, updated_br_config->network_name, sizeof(updated_br_config->network_name)) != 0) {
        status = ws_iface->validate_network_name(updated_br_config->network_name);
        if (status != MESH_ERROR_NONE) {
            tr_warn("FAILED: Received network name is not valid");
            return NM_STATUS_FAIL;
        }
        status = ws_iface->set_network_name(updated_br_config->network_name);
        if (status != MESH_ERROR_NONE) {
            tr_warn("FAILED to set network name to Nanostack");
            return NM_STATUS_FAIL;
        }
        tr_info("SET Network Name: %s", updated_br_config->network_name);
    }
    ...
    updated_br_config->resource_version = get_br_resource_version();
    tr_info("BR_resource_version %lu", updated_br_config->resource_version);
    /* Delay parameter are currently not used */
    updated_br_config->delay = 0;
    return NM_STATUS_SUCCESS;
}
...
static nm_status_t get_default_br_config_from_nanostack(nm_br_config_t *br_config)
{
    ...
    status = ws_iface->get_network_name(br_config->network_name);
    if (status != MESH_ERROR_NONE) {
...
static nm_status_t nm_configure_border_router(void)
{
    ...
    if (update_br_config_to_nanostack(&default_br_config, &modified_br_config) == NM_STATUS_FAIL) {
...
{
    if (border_router_started == true) {
        tr_debug("Border Router is already started");
        return NM_STATUS_SUCCESS;
    }

    if (nm_configure_border_router() == NM_STATUS_FAIL) {
        tr_error("FAILED to configure Border Router");
        return NM_STATUS_FAIL;
    }

    if (ws_br.start(mesh_network, backhaul_interface) == MESH_ERROR_NONE) {
        border_router_started = true;
        tr_info("Border Router Started");
        return NM_STATUS_SUCCESS;
    }
...
{
    SocketAddress sa;
    int status = mesh_network->get_ip_address(&sa);
    if (status != 0) {
        tr_debug("Mesh Iface get_ip_address failed with %d", status);
        return NM_STATUS_FAIL;
    }
...
/*
 * "nm_iface_br_down" function is called to down/disconnect the border router interface
 */
void nm_iface_br_down(void)
{
    ...
    tr_info("Border Router interface is disconnected");
}
...
nm_status_t nm_res_set_br_config(uint8_t *data, size_t length)
{
    ...
    if (update_br_config_to_nanostack(&kvstored_br_config, &received_br_config) == NM_STATUS_FAIL) {
        tr_warn("FAILED to set Border Router config to Nanostack");
        return NM_STATUS_FAIL;
    }
...
nm_status_t nm_res_get_routing_table(uint8_t **datap, size_t *length)
{
    ...
    uint8_t *cborise_data = NULL;
    size_t cborise_len = 0;
    uint16_t routing_table_length = 0;
    ...
    if (br_info.device_count == 0) {
        tr_warn("NO device has joined with Border Router");
        return NM_STATUS_FAIL;
    }
    ...
    if (route_info_mem.size < required_length) {
        if (route_info_mem.buf) {
            nm_dyn_mem_free(route_info_mem.buf);
        }
        route_info_mem.size = ((required_length / ROUTE_INFO_MIN_BLOCK_SIZE) + 1) * ROUTE_INFO_MIN_BLOCK_SIZE;
        route_info_mem.buf = nm_dyn_mem_alloc(route_info_mem.size);
        tr_info("Allocating New buffer for routing table of Size: %d", route_info_mem.size);
    }
    cborise_data = (uint8_t *)route_info_mem.buf;
    if (cborise_data == NULL) {
        tr_error("FAILED to allocate memory for Cborise Route Info");
        return NM_STATUS_FAIL;
    }

    route_info = (ws_br_route_info_t *)(cborise_data + ROUTING_TABLE_CBOR_OVERHEAD);
    ...
    if (nm_routing_table_to_cbor((uint8_t *)route_info, routing_table_length, cborise_data, &cborise_len) == NM_STATUS_FAIL) {
