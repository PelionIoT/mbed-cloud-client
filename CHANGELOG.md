...
### Release 4.7.2-multicast

### Device Management Client

- Added fallback timer for asynchronous DNS requests (`PAL_DNS_API_VERSION` = 2). Client will wait 10 minutes for a response to DNS query before aborting the request and raising a DNS error event.
...
- Mesh update: Critical messages are now sent multiple times to improve success rate for Mesh campaigns.
  - NOTE: due to the multiple message sending, minimum configurable activate delay is 60 minutes. BR will enforce the minimum value by silently reverting to 60 minute delay, but longer delays will remain unchanged.
- Fixed handling of duplicate messages during failed bootstrap recovery phase. Client was failing internally on 'NotAllowed' error instead of proceeding to bootstrap.
...
* [Linux] Fix async sockets after reboot done with execv. After reboot with execv signal handler does not return and signals might be blocked. As signal mask is inherited we need to explicitly unblock SIGIO and SIGUSR1.
